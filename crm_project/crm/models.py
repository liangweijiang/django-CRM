...
class Customer(models.Model):
    ...
    def __str__(self):
        ...
        except ConsultRecord.DoesNotExist:
            return "%s" % self.qq
    ...
    def enroll_link(self):
        if not self.enrollment_set.exists():
            return mark_safe('<a href="{}">添加</a>'.format(
                reverse('crm:add_enrollment', args=(self.id,))))
        else:
            return mark_safe(
...
class ClassList(models.Model):
    ...
    def __str__(self):
        return "{}{}班({})".format(self.get_course_display(), self.semester, self.campuses)
...
class Enrollment(models.Model):
    ...
    contract_agreed = models.BooleanField("已认真阅读完培训协议并同意全部协议内容", default=False)
...
class CourseRecord(models.Model):
    ...
    def __str__(self):
        return '{}<{}>'.format(self.re_class, self.course_title)
